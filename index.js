...
function getUsersFromDatabase(array) {
  return Array.from(array, (value) => ({
    id: value.id,
    firstname: value.name.toUpperCase(),
    years: value.age,
  }));
...
function findElementIndexes(arr, element) {
  ...
  const newArr = [];
  newArr.push(arr.indexOf(2), arr.lastIndexOf(2));
  return newArr;
...
function customFill(arr, value, start = 0, end = arr.length) {
  if (!Array.isArray(arr) || start >= arr.length || end > arr.length) {
    return false;
  }
...
console.log(customUnshift([2, 3, 4, 5], 1, 0)); // Виведе { C: 4, newLength: 6, arr: [ 1, 0, 2, 3, 4, 5 ] }
...
function customAt(arr, index) {
  return `element is ${typeof arr.at(index)}`;
...
function customIncludes(arr, element) {
  const include = arr.includes(element);
  const filter = arr.filter((value) => value === element);
  return `${filter.length}, ${include}`;
  // Перевіряємо, чи вхідний параметр є масивом
